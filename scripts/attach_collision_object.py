...
#ATTACHED_COLLISION_OBJECT_TOPIC = "/korus/attached_collision_object"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Attach an object from the household objects database to the robot in the MoveIt planning scene')
    parser.add_argument('--detach', dest='detach', action='store_true', default=False, help='Detach all objects from the robot')
    args=parser.parse_args()
    
    pub_att_col_obj = rospy.Publisher(ATTACHED_COLLISION_OBJECT_TOPIC,
                                      moveit_msgs.AttachedCollisionObject,
                                      latch = True)
    while (pub_att_col_obj.get_num_connections() < 1) and not rospy.is_shutdown():
        rospy.loginfo("Waiting for planning scene listening to '" + str(ATTACHED_COLLISION_OBJECT_TOPIC) + "' ...")
        rospy.sleep(0.5)
    ...
    if args.detach:
        ...
        shape_pose.position.z = 0.0 + object_shape.dimensions[0] / 2 # reference is at the centre of the mesh
    ...
    pub_att_col_obj.publish(att_col_obj)
